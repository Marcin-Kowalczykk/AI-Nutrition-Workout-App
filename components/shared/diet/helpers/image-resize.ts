...
export const resizeImageForUpload = (file: File): Promise<File> => {
  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const ratio =
        maxSourceDimension > MAX_UPLOAD_IMAGE_DIMENSION
      ...
      const exportAsJpeg = (quality: number) =>
      ...
      (async () => {
        ...
        for (const quality of qualities) {
          ...
          if (!blob) continue;
          if (
            blob.size <= MAX_UPLOAD_IMAGE_BYTES ||
