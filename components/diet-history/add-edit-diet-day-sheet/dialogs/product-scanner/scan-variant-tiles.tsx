...
interface ScanVariantTilesProps {
  ...
  /** When true, shows a single non-interactive per-100g tile (no choice available) */
  readonly?: boolean;
...
export const ScanVariantTiles = ({
  ...
  const tileBase = "relative flex flex-col gap-1 rounded-xl border p-3 transition-colors";
  const selectedStyle = "border-primary-element bg-primary-element/5";
  const unselectedStyle = "border-border hover:border-primary-element/50 cursor-pointer";

  // Single readonly tile — per-100g only, no choice
  if (readonly || !wp) {
    return (
      <div className="grid grid-cols-1 gap-2">
        <div
          data-variant="per_100g"
          className={`${tileBase} ${selectedStyle}`}
        >
